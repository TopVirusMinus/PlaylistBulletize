...
const Results = ({ list }) => {
  ...
  return (
    <div>
      ...
      <div class="read-only overflow-scroll w-fit max-w-80% mt-5 font-semibold block h-80 p-2.5 outline-none text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">
        {listOfVideos.map((l) => (
          <div>{l}</div>
        ))}
      </div>
